 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const xe=new Map,ct=new Map;function ir(n,e){try{n.container.addComponent(e)}catch(t){ae.debug(`Component ${e.name} failed to register with FirebaseApp ${n.name}`,t)}}function ye(n){const e=n.name;if(ct.has(e))return ae.debug(`There were multiple attempts to register component ${e}.`),!1;ct.set(e,n);for(const t of xe.values())ir(t,n);return!0}function en(n,e){const t=n.container.getProvider("heartbeat").getImmediate({optional:!0});return t&&t.triggerHeartbeat(),n.container.getProvider(e)}/**
 * @license
 * Copyright 2019 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */function De(n,e,t){return`firebase:${n}:${e}:${t}`}class fe{constructor(e,t,i){this.persistence=e,this.auth=t,this.userKey=i;const{config:r,name:s}=this.auth;this.fullUserKey=De(this.userKey,r.apiKey,s),this.fullPersistenceKey=De("persistence",r.apiKey,s),this.boundEventHandler=t._onStorageEvent.bind(t),this.persistence._addListener(this.fullUserKey,this.boundEventHandler)}setCurrentUser(e){return this.persistence._set(this.fullUserKey,e.toJSON())}async getCurrentUser(){const e=await this.persistence._get(this.fullUserKey);return e?oe._fromJSON(this.auth,e):null}removeCurrentUser(){return this.persistence._remove(this.fullUserKey)}savePersistenceForRedirect(){return this.persistence._set(this.fullPersistenceKey,this.persistence.type)}async setPersistence(e){if(this.persistence===e)return;const t=await this.getCurrentUser();if(await this.removeCurrentUser(),this.persistence=e,t)return this.setCurrentUser(t)}delete(){this.persistence._removeListener(this.fullUserKey,this.boundEventHandler)}static async create(e,t,i="authUser"){if(!t.length)return new fe(q(Pt),e,i);const r=(await Promise.all(t.map(async I=>{if(await I._isAvailable())return I}))).filter(I=>I);let s=r[0]||q(Pt);const c=De(i,e.config.apiKey,e.name);let l=null;for(const I of t)try{const h=await I._get(c);if(h){const N=oe._fromJSON(e,h);I!==s&&(l=N),s=I;break}}catch{}const g=r.filter(I=>I._shouldAllowMigration);return!s._shouldAllowMigration||!g.length?new fe(s,e,i):(s=g[0],l&&await s._set(c,l.toJSON()),await Promise.all(t.map(async I=>{if(I!==s)try{await I._remove(c)}catch{}})),new fe(s,e,i))}}/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class Y extends Ce{constructor(){super("twitter.com")}static credential(e,t){return ce._fromParams({providerId:Y.PROVIDER_ID,signInMethod:Y.TWITTER_SIGN_IN_METHOD,oauthToken:e,oauthTokenSecret:t})}static credentialFromResult(e){return Y.credentialFromTaggedObject(e)}static credentialFromError(e){return Y.credentialFromTaggedObject(e.customData||{})}static credentialFromTaggedObject({_tokenResponse:e}){if(!e)return null;const{oauthAccessToken:t,oauthTokenSecret:i}=e;if(!t||!i)return null;try{return Y.credential(t,i)}catch{return null}}}Y.TWITTER_SIGN_IN_METHOD="twitter.com";Y.PROVIDER_ID="twitter.com";/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
...
`},Wo=(n,e,t)=>{n.innerHTML=`
  ...
  `},jo=(n,e)=>{let t=e;e==="fwupdate"&&(t="Перезавантаження для оновлення ПЗ"),n.innerHTML=`
    <div class="charger">
      <img src="${ue}" class="logo" alt="Vite logo" />
  ...
  `},zo=(n,e,t)=>{let i="Інше";e==="fwupdate"&&(i="Оновлення ПЗ"),n.innerHTML=`
    <div class="charger">
      <img src="${ue}" class="logo" alt="Vite logo" />
  ...
  `},Go=(n,e,t,i,r)=>{const s=t.uid;console.log("---- uid=",s),console.log("charger=",Co);let c=setTimeout(()=>{xo(n,e,t,i,r)},15e3);r.onMessageArrived=l=>{switch(console.log("onMessageArrived",l),clearTimeout(c),l.status){case"init":Bo(n,e,t,i,r);break;case"ready":Fo(n,e,t,i,r);break;case"busy":Vo(n,e,t,i,r,l);break;case"charging":$o(n,e,t,i,r,l);break;case"error":Wo(n,l.code,l.reason);break;case"off":jo(n,l.reason);break;case"maintenance":zo(n,l.event,l.message);break;case"ignore_me":break;default:Ho(n)}},Uo(n,e,t,i,r)},Ko=document.getElementById("app"),Jo=n=>{if(console.log("App:element",n),!ie)return To(n);const e=new yo,t=new wo("charger_"+ie);t.onMQTTConnect=()=>{console.log("onMQTTConnect"),t.subscribe("charger/"+ie+"/status"),t.subscribe("charger/"+ie+"/data");const i=e.getCurrentUser();i&&t.publish("charger/"+ie+"/commands",`hello:${i.uid}`)},t.onMQTTLost=()=>(console.log("onMQTTLost. TODO: reconnect"),qo(n)),e.onAuthStateChanged(i=>(console.log("user=",i),i?(t.id="charger_"+ie+"_"+i.uid,t.connect(),Go(n,ie,i,e,t)):(t.disconnect(),So(n,e))))};Jo(Ko);
